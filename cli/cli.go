...
func usage() {
	...
	stderr("             [--duplicate-of <item id>]\n")
	stderr("        Link an institution. Prints an address to open in a browser, carrying\n")
	stderr("        a key good for that run only. Each login consumes one Item, and a\n")
	stderr("        production account allows ten for its lifetime.\n")
	stderr("        Sandbox sign-in is user_good / pass_good.\n\n")
	stderr("        Refuses by default to link an institution that already has an Item,\n")
	stderr("        since re-linking a login you already have creates a duplicate rather\n")
	stderr("        than recovering the first, and spends a slot that never returns.\n\n")
	stderr("        An Item is one login, though, not one institution: two logins at one\n")
	stderr("        bank are legitimately two Items. --duplicate-of names the existing\n")
	stderr("        Item the new one sits beside, and permits exactly that one duplicate\n")
	stderr("        — pick a different already-linked bank in the browser and it still\n")
	stderr("        refuses. Get the item ID from plaid-items.\n\n")
	stderr("        It cannot tell you are adding a *new* login rather than re-linking\n")
	stderr("        one you have. Nothing can: a login's identity is not known until\n")
	stderr("        after the exchange, and the exchange is what spends the slot. With\n")
	stderr("        two Items already at an institution, naming either permits a third.\n\n")
